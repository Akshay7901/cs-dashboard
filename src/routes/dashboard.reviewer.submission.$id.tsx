...
function ReviewerSubmission() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF6EE] font-sans text-stone-800">
      ...
      {/* Two-pane layout */}
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-6 px-6 py-6 lg:grid-cols-2">
        {/* LEFT — Review form */}
        <section className="max-h-[calc(100vh-7rem)] overflow-y-auto pr-2">
          <Link
          ...
          {comments.length === 0 ? (
            ...
            <ul className="space-y-2">
              {comments.map((c, i) => (
                <li
                  key={i}
                  className="flex items-start justify-between gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 shadow-sm"
                >
                  <div>
                    <div className="font-sans text-xs font-semibold uppercase tracking-wider text-stone-500">
                      Comment {i + 1}
                    </div>
                    <p className="mt-1 font-sans text-sm leading-relaxed text-stone-800 whitespace-pre-wrap">
                      {c}
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() =>
                      setComments((list) => list.filter((_, idx) => idx !== i))
                    }
          ...
          {showAdd ? (
            <div className="mt-4 rounded-xl border border-sky-300 bg-white p-3">
              <textarea
                ...
                className="w-full resize-y rounded-lg border border-stone-200 bg-white px-3 py-2 font-sans text-sm text-stone-800 placeholder:text-stone-400 focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-100"
              />
              <div className="mt-2 flex items-center justify-end gap-2">
                <button
                  ...
                  className="rounded-lg px-3 py-1.5 font-sans text-sm text-stone-600 hover:bg-stone-100"
                >
                  Cancel
                </button>
                <button
        ...
        <section className="max-h-[calc(100vh-7rem)] space-y-5 overflow-y-auto pr-2">
          {/* Title card */}
